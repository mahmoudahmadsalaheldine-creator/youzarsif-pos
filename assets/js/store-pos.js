// Touch POS (Store) — logic extracted from admin/store-pos.php.
// Expects a window.POS_DATA = { stockMap, baseRate, storeId, lastSale } object,
...
(function () {
    ...
    let rate = baseRate;
    const storeId = window.POS_DATA.storeId;
    ...
    function updateStockUi() {
        document.querySelectorAll('.pos-card').forEach(function (card) {
            ...
            card.style.opacity = stock <= 0 ? '.4' : '1';
        });
    }
        ...
        fetch(document.getElementById('saleForm').getAttribute('action'), {
            ...
            .then(function (data) {
                ...
                Object.values(chargedCart).forEach(function (l) {
                    stockMap[l.id] = Math.max(0, (stockMap[l.id] || 0) - l.qty);
                });
                cart = {};
                document.getElementById('paidUsd').value = 0;
                document.getElementById('paidLbp').value = 0;
