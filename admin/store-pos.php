...
$cashier = requireCashierLogin($pdo, BASE_URL . "auth/login.php");
$storeId = (int) $cashier["location_id"];
$storeName = $cashier["location_name"];
...
// Only products actually transferred (in stock) at THIS store are shown — a cashier
// never sees the full factory catalog, only what's physically been sent to them.
try {
    $stmt = $pdo->prepare("
        SELECT fp.product_id, fp.product_name, fp.product_code, fp.selling_price_usd, u.abbreviation, c.category_name,
            COALESCE(SUM(CASE WHEN psm.direction = 'in' AND psm.status = 'active' THEN psm.quantity WHEN psm.direction = 'out' AND psm.status = 'active' THEN -psm.quantity ELSE 0 END), 0) AS stock
        FROM finished_products fp
        INNER JOIN units u ON fp.unit_id = u.unit_id
        INNER JOIN categories c ON fp.category_id = c.category_id
        LEFT JOIN product_stock_movements psm ON psm.product_id = fp.product_id AND psm.location_id = :loc
        WHERE fp.status = 'active'
        GROUP BY fp.product_id, fp.product_name, fp.product_code, fp.selling_price_usd, u.abbreviation, c.category_name
        HAVING stock > 0
        ORDER BY fp.product_name ASC
    ");
    $stmt->execute([":loc" => $storeId]);
...
foreach ($products as $p) {
    $stockMap[$p['product_id']] = (float) $p['stock'];
}
...
<div class="pos-root">
    ...
    <aside class="pos-rail" id="posRail">
        ...
        <div class="pos-rail-section">Store POS</div>
        <a href="../auth/logout.php" class="pos-rail-icon">
            <svg viewBox="0 0 24 24" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:18px; height:18px; stroke:currentColor; fill:none; flex:none;"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>
            <span class="pos-rail-icon-label">Logout</span>
        </a>
        <div style="flex:1;"></div>
        <div style="padding:16px 22px; border-top:1px solid rgba(248,241,232,0.1); display:flex; align-items:center; gap:12px;">
            <div style="width:36px; height:36px; flex:none; border-radius:50%; background:#5a3a23; display:flex; align-items:center; justify-content:center; font-family:var(--font-display); font-weight:700; font-size:14px;"><?= htmlspecialchars(mb_substr($cashierName, 0, 1)); ?></div>
            <div style="font-size:13px; font-weight:600; opacity:.75;"><?= htmlspecialchars($cashierName); ?></div>
        </div>
    </aside>
    ...
    <main class="pos-main">
        <header class="pos-topbar">
            ...
            <div style="display:flex; flex-direction:column; gap:2px;">
                ...
                <div style="font-size:12px; color:var(--text-faint);">Cashier · <?= htmlspecialchars($cashierName); ?></div>
            </div>

            <div style="display:flex; align-items:center; gap:9px; margin-left:8px; background:var(--bg); padding:7px 14px; border-radius:9999px;">
                <span style="font-size:16px;">⌂</span>
                <span style="font-family:var(--font-display); font-weight:700; font-size:13px; color:var(--text);"><?= htmlspecialchars($storeName); ?></span>
            </div>

            <div class="spacer"></div>

            <a href="../auth/logout.php" style="display:flex; align-items:center; gap:6px; background:var(--bg); padding:7px 14px; border-radius:9999px; text-decoration:none; color:var(--text-muted); font-family:var(--font-body); font-weight:600; font-size:13px;"><svg viewBox="0 0 24 24" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:14px; height:14px; stroke:currentColor; fill:none;"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg> Logout</a>
...
<script src="../assets/js/store-pos.js?v=2"></script>
